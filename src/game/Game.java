 ...
 */
    private void initializeStage() {
        ...
        mainStage.setTitle("Jumper");
    }

    /**
     * Tworzenie nowej gry
     *
     * @param menuStage jest to zmienna która wyswietla menu
     */
     ...
     * W lewym górnym roku są przedstawione schodki i wynik
     * Po przekorczniu schodka wielokrotności 100 wyświetlany jest napis "FASTER"
     */
    ...
    /*
    Okno po zakończeniu gry.
    Można zapisać swój rekord.
     */
    ...
    private void animatePlayer() {
        ...
        if (player.getPositionX() + player.getVelocityX() < 560 && player.getPositionX() + player.getVelocityX() > -0)
        ...
        } else if (player.getVelocityY() != 0 && player.getVelocityX() != 0) // sprite skakania w raz z ruchem poziomym
        {
            ...
            player.animation.setColumns(1);
            player.animation.setCount(1);
        } else { // jezeli nie spelniono powyzszych to po prostu sprite stania na schodku
        ...
        if (isSpaceBarPressed) // jezeli wcisnieto spacje to skok w gore
        {
            if (player.isOnGround() == true) {
                if (player.getVelocityX() > 0)
                    player.setVelocityY(-16 + player.getVelocityX() * (-0.75)); // bonusy za predkosc pozioma
                else if (player.getVelocityX() < 0)
                    player.setVelocityY(-16 + player.getVelocityX() * 0.75); // bonusy za predkosc pozioma
                else
                    player.setVelocityY(-18); // skok w pionie bez prekosci poziomen (slabszy niz bez predkosci poziomej)
